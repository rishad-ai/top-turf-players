...
function GoalBoots({ count }: { count: number }) {
  if (!count || count < 1) return null;
  return (
    <span className="text-[10px] leading-none" title={`${count} goal${count === 1 ? "" : "s"}`}>
      {"\u26bd".repeat(count)}
    </span>
  );
}
...
export function DashboardMatchCard({
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (isMobile()) {
        ...
        if (win) {
          win.document.write(
            '<title>TOP Turf Players ' + matchDate + '</title><meta name="viewport" content="width=device-width,initial-scale=1"><body style="margin:0;background:#0E1B33;display:flex;flex-direction:column;align-items:center;gap:12px;padding:16px;font-family:system-ui"><p style="color:#fff;font-size:14px;text-align:center">Press and hold the image, then choose <b>Save to Photos</b>.</p><img src="' + dataUrl + '" style="max-width:100%;height:auto;border-radius:12px"/></body>'
          );
          win.document.close();
        } else {
          setNote("Allow pop-ups to save the image, or try a regular browser tab.");
        }
      } else {
      ...
      setNote("Couldn't generate the image. Try again in a moment.");
    } finally {
  ...
  return (
    <div>
      ...
      {note && <p className="mt-1 text-center text-xs text-ink-muted">{note}</p>}
    </div>
  );
}
